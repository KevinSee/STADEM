...
* `dplyr` and `lubridate` can both be installed by installing the `tidyverse` package
* `httr2`
* `rjags`
* `boot`
...
The STADEM package relies on many functions from two other packages, `dplyr` for many data manipulation functions and `lubridate` for dates and intervals. It uses the `httr2` package for many queries of data. It also depends upon the `rjags` package to run the JAGS program from within R.
